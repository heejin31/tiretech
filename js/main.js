...
$mainBtn.click(function(){
    if($(window).width() < 1170){  //모바일상태였을때만
        if( !$(this).parents('li').hasClass('on')){   //클릭한 a의 부모 li에 on클래스가 없을때
            $('.sub').slideUp(200);                  //모든 서브들이 업되고
            $('.navbar_gnb > li').removeClass('on');
            $(this).siblings('.sub').slideDown(500);   //클릭한 애만 슬라이드 다운됨
...
/*
mouseenter/moseleave   - 자식요소가 있다면 해당 자식요소 영역은 제외
mousover/mouseout      - 자식요소가 있다면 해당 자식요소까지 포함(자식요소를 더 확실하게 인식)
